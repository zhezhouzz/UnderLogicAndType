...
Proof.
  (* The old proof relied on Leibniz equality for denotations under
     environment agreement.  The new TypeDenotation route intentionally only
     exposes models-level / store-level transport, so this context theorem
     should be rebuilt from named weak congruence lemmas rather than by
     unfolding the formula structure here. *)
Admitted.
