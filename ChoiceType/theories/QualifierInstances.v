...
#[global] Instance SubstFresh_qualifier : SubstFresh type_qualifier.
Proof.
  intros q x v Hfresh.
  exact (qual_subst_fresh x v q Hfresh).
Qed.

#[global] Instance FvOfSubst_qualifier : FvOfSubst type_qualifier.
Proof.
  intros x v [B d p]. unfold subst_one, stale; simpl.
  destruct (decide (x ∈ d)); set_solver.
Qed.

#[global] Instance FvOfSubstClosed_qualifier : FvOfSubstClosed type_qualifier.
Proof.
  intros x v [B d p] _. unfold subst_one, stale; simpl.
  destruct (decide (x ∈ d)); set_solver.
Qed.
