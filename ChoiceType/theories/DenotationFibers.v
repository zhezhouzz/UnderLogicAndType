...
Lemma fib_vars_singleton x p :
  fib_vars {[x]} p = FFibVars (lvars_of_atoms {[x]}) p.
Proof.
  reflexivity.
Qed.
...
Proof.
  unfold fib_vars. simpl. rewrite lvars_fv_of_atoms. set_solver.
Qed.

Lemma fib_vars_formula_fv X p :
  formula_fv (fib_vars X p) = X ∪ formula_fv p.
Proof.
  unfold fib_vars. simpl. rewrite lvars_fv_of_atoms. reflexivity.
Qed.
...
Lemma fib_vars_obligation_insert_fresh_to_fib
  ...
  fib_vars_obligation X (fib_vars {[x]} p) ρ m.
Proof.
