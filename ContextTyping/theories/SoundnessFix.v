...
Lemma fundamental_fix_case Σ Γ φx τ vf b t (L : aset) :
  ...
  ctx_denote_under Σ Γ ⊫
    ty_denote_under Σ Γ (CTArrow (over_ty b φx) τ)
      (tret (vfix (TBase b →ₜ t) vf)).
Proof.
  (* Result-first Arrow denotation makes the first [FForall] bind the function
     value produced by [ret fix], not the user argument. The old proof opened
     that binder as the argument and then called [fix_arrow_open_arg_normalize].
     The replacement proof should discharge the outer result graph for
     [ret fix], then run the old opened-argument/fix-body route inside
