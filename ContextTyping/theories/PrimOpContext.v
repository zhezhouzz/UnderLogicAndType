...
Proof.
  ...
  - change (x ∈ dom (σx : gmap atom value)) in Hxσx.
    ...
    change ((σ : gmap atom value) !! x =
      (σx : gmap atom value) !! x).
  ...
  - change (x ∉ dom (σx : gmap atom value)) in Hxσx.
    ...
    change (x ∈ dom (σx : gmap atom value)) in Hxσx_dom.
    apply elem_of_dom in Hxσx_dom as [vx Hlook].
    rewrite Hxσx in Hlook. discriminate.
Qed.

Lemma lstore_lift_restrict_singleton_free_eq
    y D (s : LStoreOn D) (σ : StoreT) v :
  D = ({[LVFree y]} : lvset) ->
  σ !! y = Some v ->
  (lso_store s : LStore) !! LVFree y = Some v ->
  storeA_restrict
    (lstore_lift_free (store_restrict σ (lvars_fv D)) : LStore) D =
  lso_store s.
Proof.
  intros HD Hσy Hsy.
  subst D.
...
Lemma lstore_in_lworld_on_singleton_free_lookup
  ...
  exists σ,
    (m : WorldT) σ /\
    (lso_store s : LStore) !! LVFree y = σ !! y.
Proof.
  intros HD Hmem.
  subst D.
  ...
  pose proof (f_equal (fun st => (st : LStore) !! LVFree y) Hrestrict_l)
    as Hyl.
  cbn [lso_store storeAO_store] in Hyl.
  change ((storeA_restrict (lstore_lift_free τ : LStore)
    ({[LVFree y]} : lvset) : LStore) !! LVFree y =
    (lso_store s : LStore) !! LVFree y) in Hyl.
  replace ((storeA_restrict (lstore_lift_free τ : LStore)
      ({[LVFree y]} : lvset) : LStore) !! LVFree y)
    with ((τ : gmap atom value) !! y) in Hyl.
  2:{
    symmetry.
    rewrite (storeA_restrict_lookup
      (K := logic_var) (V := value)
      (lstore_lift_free τ : LStore) ({[LVFree y]} : lvset) (LVFree y)).
  ...
  rewrite (storeA_restrict_lookup
    (K := atom) (V := value)
    (σ : gmap atom value) (lvars_fv ({[LVFree y]} : lvset)) y) in Hyτ.
