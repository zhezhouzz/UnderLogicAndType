...
Proof.
  intros Hctx Hσ.
  pose proof (ctx_denote_under_bind_inv ∅ x (primop_arg_ty (concrete_Φ op)) m Hctx)
    as Harg.
  unfold ty_denote in Harg.
  eapply (ty_denote_ret_fvar_base_const
    (cty_depth (primop_arg_ty (concrete_Φ op)))
    (atom_env_to_lty_env
      (<[x := erase_ty (primop_arg_ty (concrete_Φ op))]>
        (store_restrict ∅ (ctx_fv (CtxBind x (primop_arg_ty (concrete_Φ op)))))))
    (primop_arg_ty (concrete_Φ op)) (prim_arg_base (concrete_Φ op)) x m).
...
Proof.
  ...
  pose proof (ctx_denote_under_basic_world
    (∅ : gmap atom ty) (CtxBind x (primop_arg_ty (concrete_Φ op))) m Hctx)
    as Hworld.
...
Proof.
  ...
  - intros τ Hτ.
    ...
    destruct (primop_arg_const_from_ctx op x m σ Hctx Hσ)
      as [c [Hlookup Hbase]].
    assert (Herasure :
        prim_op_type op = (prim_arg_base (concrete_Φ op), prim_ret_base (concrete_Φ op))).
    {
      unfold concrete_Φ, concrete_primop_sig.
      destruct (prim_op_type op); reflexivity.
    }
    pose proof (prim_step_total_for_base op c
      (prim_arg_base (concrete_Φ op)) (prim_ret_base (concrete_Φ op))
...
Proof.
  ...
  assert (Hstatic_full :
      m ⊨ ty_static_guard_formula
        (atom_env_to_lty_env
          (erase_ctx (CtxBind x (primop_arg_ty (concrete_Φ op)))))
        ({0 ~> x} τbody) (tprim op (vfvar x))).
  {
    ...
    - split.
      ...
      + apply expr_basic_typing_formula_models_iff.
        ...
        eapply (BTT_Op _ op _ (prim_arg_base (concrete_Φ op))
...
Proof.
  ...
  - eapply primop_opened_result_guard_formula.
    + reflexivity.
    + unfold concrete_Φ, concrete_primop_sig, primop_result_ty.
...
Proof.
  ...
  pose proof (ty_denote_gas_guard
    (cty_depth ({0 ~> x} (primop_result_ty (concrete_Φ op))))
    (atom_env_to_lty_env (erase_ctx (CtxBind x (primop_arg_ty (concrete_Φ op)))))
    ({0 ~> x} (primop_result_ty (concrete_Φ op)))
  ...
  - destruct (decide (a = x)) as [->|Hneq].
    ...
    change ((<[LVFree x := TBase (prim_arg_base (concrete_Φ op))]>
    ...
    + rewrite atom_store_to_lvar_store_lookup_free.
      change (erase_ctx (CtxBind x (primop_arg_ty (concrete_Φ op))) !! x =
...
Proof.
  ...
  eapply ctx_denote_bind_from_arg_denotation
    with (Σ := <[x := erase_ty (primop_arg_ty (concrete_Φ op))]> ∅).
  - unfold concrete_Φ, concrete_primop_sig, primop_arg_ty.
    destruct (prim_op_type op) as [arg_b ret_b].
  ...
  - rewrite primop_arg_ty_eq.
    ...
    replace (atom_env_to_lty_env
        (<[x := TBase (prim_arg_base (concrete_Φ op))]> (∅ : gmap atom ty)))
      with (<[LVFree x := TBase (prim_arg_base (concrete_Φ op))]>
