...
Proof.
        ...
        (atom_env_to_lty_env (erase_ctx Γlet))).
  {
    ...
    exact (wf_ctx_under_basic Σ Γlet Hwfctx).
  }
...
Proof.
  ...
  replace (atom_env_to_lty_env (<[x := erase_ty τ]> (ctx_erasure_under Σ Γ)))
    with (<[LVFree x := erase_ty τ]>
      (atom_env_to_lty_env (ctx_erasure_under Σ Γ))) in Harg.
  2:{ symmetry. apply atom_store_to_lvar_store_insert. }
...
Proof.
        ...
        (atom_env_to_lty_env (<[x := erase_ty τ1]> Σ))).
  {
    ...
    2:{ symmetry. apply atom_store_to_lvar_store_insert. }
    eapply basic_world_insert_of_arg.
    - apply atom_env_to_lty_env_dom_free_notin. exact HxΣ.
    - eapply res_models_extend_mono; [exact Hext|].
      eapply basic_world_formula_subenv.
      + intros v T Hv.
        unfold ctx_erasure_under.
        destruct v as [k|a].
    ...
    - replace (<[LVFree x := erase_ty τ1]> (atom_env_to_lty_env Σ))
        with (atom_env_to_lty_env (<[x := erase_ty τ1]> Σ)).
      + exact Harg_base.
  ...
  rewrite res_models_and_iff. split.
  - change (mx ⊨ basic_world_formula
      (atom_env_to_lty_env (ctx_erasure_under Σ (CtxBind x τ1)))).
    replace (ctx_erasure_under Σ (CtxBind x τ1))
      with (<[x := erase_ty τ1]> Σ).
    + exact Hworld_base.
    + unfold ctx_erasure_under. cbn [erase_ctx].
      symmetry.
      apply (storeA_union_singleton_insert_fresh
        (V := ty) (K := atom) Σ x (erase_ty τ1)).
      exact HxΣ.
  - unfold ty_denote.
    exact Harg_base.
Qed.
...
Proof.
  ...
  pose proof (ctx_denote_under_star_intro_product
    Σ Γ2 (CtxBind x τ1) m2 m1x Hc2x HΓ2 Hbind) as Hctx_body.
...
Lemma lam_arrow_arg_type_open_shift_eq
  ...
  y ∉ dom (ctx_erasure_under Σ Γ) ∪ fv_tm e ∪ fv_cty τx ∪ fv_cty τ ->
...
Lemma lam_arrow_open_arg_mid_to_bind_denotation
  ...
  my ⊨ ty_denote_gas (cty_depth τx)
    ...
    τx (tret (vfvar y)) ->
  my ⊨ ty_denote_gas (cty_depth τx)
    (atom_env_to_lty_env (<[y:=erase_ty τx]> (Σ ∪ erase_ctx Γ)))
    τx (tret (vfvar y)).
Proof.
  intros Hwf Hy Hmid.
  pose proof (context_typing_wf_ctx Σ Γ
...
Lemma lam_arrow_open_arg_to_bind_denotation
  ...
  my ⊨ ty_denote_gas (cty_depth τx)
    ...
    τx (tret (vfvar y)).
Proof.
  intros Hwf Hdom Hrestrict Hy Hbasic Harg.
  assert (HΣfresh :
      y ∉ lvars_fv
...
Proof.
  intros Hwf Hctx_my Hdom Hrestrict Hy Hbasic Harg.
  pose proof (lam_arrow_open_arg_to_bind_denotation
    Σ Γ τx τ e m my y Hwf Hdom Hrestrict Hy Hbasic Harg)
    as Hbind_den.
  cbn [ctx_denote_under].
  rewrite res_models_and_iff. split.
  - unfold ctx_erasure_under.
    assert (Hambient :
        my ⊨ basic_world_formula
          (atom_env_to_lty_env (Σ ∪ erase_ctx Γ))).
    {
      exact (ctx_denote_under_basic_world Σ Γ my Hctx_my).
    }
    replace (atom_env_to_lty_env
        ((Σ ∪ erase_ctx Γ) ∪ erase_ctx (CtxBind y τx)))
      with (<[LVFree y := erase_ty τx]>
        (atom_env_to_lty_env (Σ ∪ erase_ctx Γ))).
    2:{
      replace ((Σ ∪ erase_ctx Γ) ∪ erase_ctx (CtxBind y τx))
        with (<[y := erase_ty τx]> (Σ ∪ erase_ctx Γ)).
      - symmetry. apply atom_store_to_lvar_store_insert.
      - cbn [erase_ctx].
        symmetry. apply storeA_union_singleton_insert_fresh.
        unfold ctx_erasure_under in Hy. set_solver.
    ...
    eapply (basic_world_insert_of_arg
      (atom_env_to_lty_env (Σ ∪ erase_ctx Γ)) τx y
  ...
  my ⊨ formula_open 0 y
    ...
    (relevant_env
      ...
      (tapp_tm (tret (vlam (erase_ty τx) e)) (vfvar y)))
	    ...
	    (tapp_tm (tret (vlam (erase_ty τx) e)) (vfvar y)).
	Proof.
	  intros Hwf Hy Hbasic Harg Hzero_body Hstatic.
	  ...
	  assert (Hy_dom : y ∈ world_dom (my : WorldT)).
	  {
	    pose proof (basic_world_formula_opened_arg_world
	      (erase_ty τx) my y Hbasic) as Hbasic_y.
	    apply basic_world_formula_models_iff in Hbasic_y as [_ [Hscope _]].
	    apply Hscope.
	    apply (proj2 (lvars_fv_elem _ y)).
	    apply elem_of_dom. exists (erase_ty τx).
...
Lemma lam_arrow_result_mid_app_denotation
  ...
  my ⊨ formula_open 0 y
    ...
    (tapp_tm (tret (vlam (erase_ty τx) e)) (vfvar y)).
Proof.
	  intros Hwf Hy Hbasic Harg Hmid Hstatic.
  assert (Hzero_body :
      my ⊨ ty_denote_gas 0
        (lty_env_open_one 0 y
          (typed_lty_env_bind
...
Proof.
  ...
  set (elam := tret (vlam (erase_ty τx) e)).
	  pose proof (lam_arrow_result_mid_app_denotation
	    Σ Γ τx τ e my y Hwf Hy Hbasic Harg Hmid Hstatic) as Happ_mid.
...
Lemma lam_body_to_opened_arrow_result
  ...
  my ⊨ formula_open 0 y
    ...
    (ty_denote_gas (Nat.max (cty_depth τx) (cty_depth τ))
      ...
      (tapp_tm (tm_shift 0 (tret (vlam (erase_ty τx) e))) (vbvar 0))).
Proof.
	  intros Hwf Hy Hbasic Harg Hbody Hstatic.
...
Lemma lam_opened_arrow_result
  ...
  world_dom (my : WorldT) = world_dom (m : WorldT) ∪ {[y]} ->
  res_restrict my (world_dom (m : WorldT)) = m ->
  y ∉ L ∪ dom (ctx_erasure_under Σ Γ) ∪ fv_tm e ∪ fv_cty τx ∪ fv_cty τ ->
  my ⊨ formula_open 0 y
    (basic_world_formula (<[LVBound 0 := erase_ty τx]> ∅)) ->
	  my ⊨ formula_open 0 y
	    (ty_denote_gas (Nat.max (cty_depth τx) (cty_depth τ))
    ...
    (ty_denote_gas (Nat.max (cty_depth τx) (cty_depth τ))
      ...
      (tapp_tm (tm_shift 0 (tret (vlam (erase_ty τx) e))) (vbvar 0))).
Proof.
  intros Hwf IH Hctx Hdom Hrestrict Hy Hbasic Harg.
  assert (Hctx_comma :
      my ⊨ ctx_denote_under Σ (CtxComma Γ (CtxBind y τx))).
  {
...
Proof.
  ...
  - pose proof (ty_denote_gas_scope_from_zero_any
    ...
    exists (L ∪ dom (ctx_erasure_under Σ Γ) ∪ fv_tm e ∪ fv_cty τx ∪ fv_cty τ).
    ...
    eapply res_models_impl2_intro; [exact Hopen_scope|].
    intros Hbasic Harg.
