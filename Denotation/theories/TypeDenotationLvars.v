...
Proof.
  ...
  - destruct τ as [b φ|b φ|τ1 τ2|τ1 τ2|τ1 τ2|τ1 τ2|τ1 τ2];
    ...
    + unfold_formula_lvars_atoms.
      ...
      rewrite choice_ty_lvars_at_shift_under in Harg by lia.
      pose proof (IH (S d)
        (typed_lty_env_bind (denot_relevant_env Σ (CTArrow τ1 τ2) e)
          (erase_ty τ1))
      ...
      cbn [choice_ty_lvars_at].
      cbn [choice_ty_lvars_at] in Hrel.
      intros v Hv.
...
Proof.
  ...
  rewrite <- (choice_ty_lvars_fv_at 0 (cty_open k y τ)).
  ...
  rewrite (choice_ty_lvars_at_open 0 k y τ).
...
Proof.
  rewrite <- (formula_lvars_at_fv 0).
  transitivity (lvars_fv (tm_lvars_at 0 e ∪ choice_ty_lvars_at 0 τ)).
  - apply lvars_fv_mono.
    apply formula_lvars_at_denot_ty_lvar_gas_subset_relevant.
  - rewrite lvars_fv_union.
    change (tm_lvars_at 0 e) with (tm_lvars e).
    change (choice_ty_lvars_at 0 τ) with (choice_ty_lvars τ).
    rewrite tm_lvars_fv, choice_ty_lvars_fv.
    reflexivity.
Qed.
...
Lemma denot_guard_term_type_fv_scope
    Σ τ e (m : WfWorldT) :
  m ⊨ FAnd
    (choice_ty_wf_formula (denot_relevant_env Σ τ e) τ)
