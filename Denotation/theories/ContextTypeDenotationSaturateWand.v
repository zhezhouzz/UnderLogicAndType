...
Lemma denot_ty_lvar_gas_tm_result_equiv_wand_open_result_source_mid
...
Lemma denot_ty_lvar_gas_tm_result_equiv_wand_open_result_target_mid_to_goal
...
Lemma basic_world_formula_wand_open_result_source_world
  ...
  typed_total_tm_result_equiv_on Σ (CTWand τx τr) m e1 e2 ->
  res_restrict my (world_dom (m : WorldT)) = m ->
  my ⊨ basic_world_formula (denot_relevant_env Σ (CTWand τx τr) e2).
Proof.
...
Proof.
  ...
  assert (Hworld_src_prod :
      res_product n my Hc ⊨
        basic_world_formula (denot_relevant_env Σ (CTWand τx τr) e2)).
...
Lemma basic_tm_has_ltype_wand_open_result_target_fun
    (Σ : lty_env) τx τr e1 e2
    (m : WfWorldT) y :
  typed_total_tm_result_equiv_on Σ (CTWand τx τr) m e1 e2 ->
  y ∉ fv_tm e1 ∪ fv_tm e2 ->
...
Proof.
  ...
  - exact Hlc_tgt.
  - eapply basic_tm_has_ltype_wand_open_result_target_fun; eauto.
  - apply basic_value_has_ltype_arrow_open_result_target_arg.
Qed.

Lemma denot_ty_lvar_gas_zero_wand_open_result_target
    gas (Σ : lty_env) τx τr e1 e2
    (m my n : WfWorldT) y (Hc : world_compat n my) :
  typed_total_tm_result_equiv_on Σ (CTWand τx τr) m e1 e2 ->
  world_dom (my : WorldT) = world_dom (m : WorldT) ∪ {[y]} ->
  ...
  n ⊨ denot_ty_lvar_gas gas
    (lty_env_open_one 0 y
      (typed_lty_env_bind
        ...
        (erase_ty τx)))
    (cty_open 0 y (cty_shift 0 τx)) (tret (vfvar y)) ->
  res_product n my Hc ⊨ denot_ty_lvar_gas 0
    (lty_env_open_one 0 y (typed_lty_env_bind Σ (erase_ty τx)))
    (cty_open 0 y τr) (tapp_tm e2 (vfvar y)).
Proof.
  intros Hequiv Hdom Hrestrict Hyτx Hyτr Hye Hworld Hres_mid Harg.
  ...
  assert (Heq_apps :
      tm_result_equiv_on (res_product n my Hc)
        (tapp_tm e1 (vfvar y)) (tapp_tm e2 (vfvar y))).
  {
    ...
    - exact Hlc1.
    - exact Hlc2.
    - eapply tm_result_equiv_on_res_product_right.
      + pose proof (typed_total_tm_result_equiv_on_term_scope
          Σ (CTWand τx τr) m e1 e2 Hequiv) as Hscope.
...
Lemma typed_total_tm_result_equiv_on_wand_open_result_mid
    gas (Σ : lty_env) τx τr e1 e2
    (m my n : WfWorldT) y (Hc : world_compat n my) :
  typed_total_tm_result_equiv_on Σ (CTWand τx τr) m e1 e2 ->
  world_dom (my : WorldT) = world_dom (m : WorldT) ∪ {[y]} ->
...
Proof.
  ...
  - split.
    ...
    + eapply (denot_ty_lvar_gas_zero_wand_open_result_target
...
Proof.
  ...
  {
    unfold τres, esrc.
    eapply denot_ty_lvar_gas_tm_result_equiv_wand_open_result_source_mid;
      eauto.
  }
  ...
  {
    unfold τres, etgt.
    eapply denot_ty_lvar_gas_tm_result_equiv_wand_open_result_target_mid_to_goal;
      eauto.
  }
  apply Htgt_mid_to_goal.
  eapply IH.
  - unfold τres, esrc, etgt in *.
    eapply typed_total_tm_result_equiv_on_wand_open_result_mid; eauto.
  - exact Hres_mid.
Qed.
