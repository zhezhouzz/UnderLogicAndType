(** * Denotation.TypeDenotationCore

    Core helpers for the recursive choice-type denotation.

    This file contains formula multi-open support and relevant-environment
    utilities used by [TypeDenotationDefinition].  The old typed-forall,
    ...
    [ChoiceBasicDenotation] directly. *)
...
Lemma formula_open_env_choice_ty_wf_formula η Σ τ :
  open_env_fresh_for_lvars η (dom Σ ∪ choice_ty_lvars τ) ->
  open_env_values_inj η ->
  formula_open_env η (choice_ty_wf_formula Σ τ) =
  choice_ty_wf_formula (lty_env_open_lvars η Σ) (open_cty_env η τ).
Proof.
  revert Σ τ.
  ...
  - intros Σ τ Hfresh Hinj.
    ...
    pose proof (open_env_fresh_for_lvars_insert_tail η k x
      (dom Σ ∪ choice_ty_lvars τ) Hnone Hfresh) as Hfreshη.
    rewrite formula_open_env_insert_fresh by assumption.
...
Lemma choice_ty_lvars_open_cty_env η τ :
  open_env_fresh_for_lvars η (choice_ty_lvars τ) ->
  choice_ty_lvars (open_cty_env η τ) =
  lvars_open_env η (choice_ty_lvars τ).
Proof.
  revert τ.
  induction η as [|k x η Hnone Hfold IH] using fin_maps.map_fold_ind.
  ...
  - intros τ Hfresh.
    pose proof (IH τ ltac:(eapply open_env_fresh_for_lvars_insert_tail; eassumption))
      as HIH.
    change (choice_ty_lvars
      (map_fold (fun k x acc => cty_open k x acc) τ (<[k:=x]> η)) =
      lvars_open_env (<[k:=x]> η) (choice_ty_lvars τ)).
    rewrite (Hfold choice_ty (fun k x acc => cty_open k x acc)).
    fold (open_cty_env η τ).
    rewrite cty_open_vars.
    unfold choice_ty_open_lvars.
    replace (choice_ty_lvars (open_cty_env η τ))
      with (lvars_open_env η (choice_ty_lvars τ)) by (symmetry; exact HIH).
...
Proof.
  intros Hfresh.
  unfold denot_relevant_lvars.
  rewrite choice_ty_lvars_open_cty_env.
  2:{
    apply (open_env_fresh_for_lvars_mono η
      (choice_ty_lvars τ) (denot_relevant_lvars τ e)).
...
Lemma lvars_at_depth_denot_relevant_env_subset_relevant d Σ τ e :
  lvars_at_depth d (dom (denot_relevant_env Σ τ e)) ⊆
  choice_ty_lvars_at d τ ∪ tm_lvars_at d e.
Proof.
  unfold denot_relevant_env, lty_env_restrict_lvars, denot_relevant_lvars.
  change (lvars_at_depth d
    (dom (storeA_restrict (Σ : gmap logic_var ty)
    ...
    choice_ty_lvars_at d τ ∪ tm_lvars_at d e).
  rewrite storeA_restrict_dom.
  transitivity (lvars_at_depth d (choice_ty_lvars τ ∪ tm_lvars e)).
...
Proof.
  ...
  unfold denot_relevant_lvars.
  rewrite lvars_at_depth_union, choice_ty_lvars_depth, tm_lvars_depth.
  rewrite choice_ty_lvars_at_shift_under by lia.
