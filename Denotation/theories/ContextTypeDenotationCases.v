...
Lemma letd_soundness_bridge
  ...
  erase_ctx (CtxStar Γ1 Γ2) ⊢ₑ tlete e1 e2 ⋮ erase_ty τ2 ->
  denot_ctx_under Σ (CtxStar Γ1 Γ2) ⊫
    denot_static_guard_formula
...
Lemma lam_soundness_bridge
    Σ Γ τx τ e (L : aset) :
  erase_ctx Γ ⊢ₑ tret (vlam (erase_ty τx) e) ⋮ erase_ty (CTArrow τx τ) ->
  denot_ctx_under Σ Γ ⊫
    denot_static_guard_formula
...
Lemma lamd_soundness_bridge
    Σ Γ τx τ e (L : aset) :
  erase_ctx Γ ⊢ₑ tret (vlam (erase_ty τx) e) ⋮ erase_ty (CTWand τx τ) ->
  denot_ctx_under Σ Γ ⊫
    denot_static_guard_formula
      (atom_env_to_lty_env (erase_ctx Γ))
...
Lemma app_soundness_bridge
    Σ Γ τx τ v1 x :
  erase_ctx Γ ⊢ₑ tapp v1 (vfvar x) ⋮ erase_ty ({0 ~> x} τ) ->
  denot_ctx_under Σ Γ ⊫
    denot_static_guard_formula
      (atom_env_to_lty_env (erase_ctx Γ))
...
Lemma fixd_soundness_bridge
  ...
  denot_ctx_under Σ Γ ⊫
    denot_static_guard_formula
      (atom_env_to_lty_env (erase_ctx Γ))
      (CTWand τx τ) (tret (vfix (TBase b →ₜ t) vf)) ->
  (forall y, y ∉ L ->
    denot_ctx_under Σ (CtxStar Γ (CtxBind y τx)) ⊫
      denot_ty_in_ctx_under Σ (CtxStar Γ (CtxBind y τx))
        (CTArrow (fix_rec_call_ty b y τx τ) ({0 ~> y} τ))
...
Lemma match_true_soundness_bridge
    Σ Γ v τ et ef :
  erase_ctx Γ ⊢ₑ tmatch v et ef ⋮ erase_ty τ ->
  denot_ctx_under Σ Γ ⊫
    denot_static_guard_formula
      (atom_env_to_lty_env (erase_ctx Γ)) τ (tmatch v et ef) ->
  (denot_ctx_under Σ Γ ⊫
    denot_ty_in_ctx_under Σ Γ (bool_precise_ty true) (tret v)) ->
  denot_ctx_under Σ Γ ⊫
    FImpl (denot_ty_in_ctx_under Σ Γ (bool_precise_ty false) (tret v))
      FFalse ->
...
Lemma match_false_soundness_bridge
  ...
  denot_ctx_under Σ Γ ⊫
    FImpl (denot_ty_in_ctx_under Σ Γ (bool_precise_ty true) (tret v))
      FFalse ->
