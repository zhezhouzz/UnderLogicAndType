...
Proof.
  ...
  eapply (res_models_ty_denote_gas_env_agree_on gas
    ...
    (<[LVFree y := erase_ty τx]>
      (relevant_env Σ (CTArrow τx τr) e_src))
    τx (tret (vfvar y))
    (context_ty_lvars τx ∪ tm_lvars (tret (vfvar y))) my).
...
Proof.
  ...
  eapply (res_models_ty_denote_gas_env_agree_on gas
    ...
    (context_ty_lvars τx ∪ tm_lvars (tret (vfvar y))) my).
  - intros v Hv. exact Hv.
  - apply wand_arg_relevant_env_agree_inserted_body_irrel.
    exact Hyτx.
  - exact Htgt.
Qed.
    ...
    reflexivity.
	  ...
	  assert (Hopened_scope :
	      formula_scoped_in_world mf
	        (FImpl
	          (expr_result_formula_at
	            (context_ty_lvars (CTArrow τx τr) ∪ tm_lvars e_tgt)
	            e_tgt (LVFree f))
	          (arrow_value_denote_gas_with ty_denote_gas gas
	            (<[LVFree f := erase_ty (CTArrow τx τr)]>
	              (relevant_env Σ (CTArrow τx τr) e_tgt))
	            τx τr (tret (vfvar f))))).
	  {
	    pose proof (formula_scoped_forall_open_res_le mx mf f
	      _ Hscope_tgt
