...
> locally-nameless logic-variable refactor and the current
> `expr_result_formula`/lworld encoding.  It is useful for old tlet proof
...
the `gmap`/`Store` instances can become ambiguous outside `ContextStore.Store`.
