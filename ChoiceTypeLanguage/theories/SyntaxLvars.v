(** * ChoiceTypeLanguage.SyntaxLvars

    Lvar support computations for choice syntax. *)

From ChoiceTypeLanguage Require Export SyntaxCore.
...
Proof.
  induction τ in d, k |- *; cbn [choice_ty_lvars_at].
...
Proof.
  unfold choice_ty_lvars, choice_ty_open_lvars.
...
Proof.
  induction τ in d |- *; unfold fv_cty, choice_ty_lvars in *;
    cbn [choice_ty_lvars_at] in *.
...
Proof.
  intros Hsub.
  transitivity (lvars_shift_from 0 (choice_ty_lvars_at 1 τ) ∪ {[LVBound 0]}).
  - apply choice_ty_lvars_at_succ_body.
  - set_solver.
Qed.
...
Proof.
  induction τ in d, k |- *; cbn [choice_ty_lvars_at cty_shift].
  - replace (S (d + k)) with (S d + k) by lia.
    rewrite qual_shift_from_vars.
...
Proof.
  unfold choice_ty_lvars, choice_ty_shift_lvars.
  change (cty_shift 0 τ) with (cty_shift (0 + 0) τ).
  apply choice_ty_lvars_at_shift.
Qed.
