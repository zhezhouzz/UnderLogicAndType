(** * ChoiceTypeLanguage.SyntaxOpen

    Open/shift structural laws for choice syntax. *)

From ChoiceTypeLanguage Require Export SyntaxLvars.
...
Proof.
  ...
  - rewrite IHτ1, IHτ2 by (try exact Hxy; unfold fv_cty in *;
      cbn [choice_ty_lvars choice_ty_lvars_at] in *; rewrite lvars_fv_union in *; set_solver).
    reflexivity.
  - rewrite IHτ1, IHτ2 by (try exact Hxy; unfold fv_cty in *;
      cbn [choice_ty_lvars choice_ty_lvars_at] in *; rewrite lvars_fv_union in *; set_solver).
    reflexivity.
  ...
  - assert (Hx1 : x ∉ fv_cty τ1).
    { intros Hbad. apply Hx. unfold fv_cty, choice_ty_lvars.
      cbn [choice_ty_lvars_at]. rewrite lvars_fv_union.
      apply elem_of_union. left. rewrite choice_ty_lvars_fv_at. exact Hbad. }
    assert (Hy1 : y ∉ fv_cty τ1).
    { intros Hbad. apply Hy. unfold fv_cty, choice_ty_lvars.
      cbn [choice_ty_lvars_at]. rewrite lvars_fv_union.
      apply elem_of_union. left. rewrite choice_ty_lvars_fv_at. exact Hbad. }
    assert (Hx2 : x ∉ fv_cty τ2).
    { intros Hbad. apply Hx. unfold fv_cty, choice_ty_lvars.
      cbn [choice_ty_lvars_at]. rewrite lvars_fv_union.
      apply elem_of_union. right. rewrite choice_ty_lvars_fv_at. exact Hbad. }
